...
@MainActor final class AskUITests: XCTestCase {
    func testMeetingQuestionOpensOriginalEvidenceAndUnavailableIsHonest() {
        for unavailable in [false, true] {
            ...
            XCTAssertTrue(app.staticTexts["Synthetic ask fixture"].waitForExistence(timeout: 5))
            ...
            if unavailable {
                ...
                // Scroll the Form; answer content can sit below the fold.
                for _ in 0..<6 { app.swipeUp() }
                let draft = app.staticTexts["Draft answer. Verify each claim against its original citations."]
                let reviewed = app.staticTexts.matching(NSPredicate(format: "label BEGINSWITH %@", "Reviewed")).firstMatch
                let claim = app.staticTexts["The launch review is on Friday."]
                let citation = app.buttons["askCitation"].firstMatch
                XCTAssertTrue(
                    draft.waitForExistence(timeout: 15) || reviewed.waitForExistence(timeout: 1) || claim.waitForExistence(timeout: 1) || citation.waitForExistence(timeout: 1),
                    "Expected Ask to finish with a reviewed answer"
                )
                XCTAssertFalse(app.staticTexts["askProblem"].exists)
                XCTAssertTrue(claim.waitForExistence(timeout: 10), "Expected claim/citation text from the typed note")
