...
export default function MeetingView({
  ...
  /** Names assigned to this meeting's speakers (calendar/manual, self aside). */
  const [participants, setParticipants] = useState<MeetingParticipant[]>([])
  /** Speaker id whose name is being edited in the transcript, if any. */
  ...
  const [exitAction, setExitAction] = useState<'idle' | 'saving' | 'discarding'>('idle')
  ...
  const titleWasAutoGeneratedRef = useRef(false)
  ...
  /** Suppresses delayed editor/session writes after an empty draft is deleted. */
  ...
  useEffect(() => {
    ...
    void window.meetings
      ...
      .then((record) => {
        ...
        // Preserve the generated transcript baseline when reopening notes.
  ...
  /**
   * The roster labels are resolved against: the meeting's own names plus the
   * user's profile name, so an ad-hoc recording reads "Sean" / "Them"
   * instead of "You" / "Them" without anything being assigned by hand.
   */
  const roster = useMemo(
    () => withSelfParticipant(participants, settings?.profileName ?? ''),
    [participants, settings?.profileName]
  )
  ...
  const startRecording = (): void => {
    if (ACTIVE_PHASES.includes(stateRef.current.phase) || startPendingRef.current) return
  ...
  const currentDraftContent = (): DraftContent => ({
    ...
    enhancedMarkdown,
    segments: allSegments,
    partialTranscript: Object.values(stateRef.current.partials)
  })

  const captureFreshNotes = (): void => {
    if (editor && docViewRef.current === 'notes') {
      roughMarkdownRef.current = docToMarkdown(editor.getJSON())
    }
    if (saveTimerRef.current !== null) {
  ...
  const saveDraftBeforeExit = async (keepEmpty = false): Promise<boolean> => {
    captureFreshNotes()
    const content = currentDraftContent()
    const kind = content.kind ?? 'meeting'
    const nextTitle = deriveDraftTitle(content) ?? (keepEmpty ? emptyDraftTitle(kind) : '')
    const generatedFromContent = isGenericDraftTitle(content.title) && nextTitle.length > 0
    if (nextTitle !== titleValueRef.current) {
      titleValueRef.current = nextTitle
      setTitle(nextTitle)
    }
    if (generatedFromContent) titleWasAutoGeneratedRef.current = true
    try {
      await window.meetings.upsert({
        id: meetingId,
        title: nextTitle,
  ...
  const goBack = async (): Promise<void> => {
    captureFreshNotes()
    if (newDraftNeedsExitDecision(isNewDraft, currentDraftContent())) {
      setExitDecisionOpen(true)
      return
    }
    if (await saveDraftBeforeExit()) onBack()
  }

  const saveEmptyDraft = async (): Promise<void> => {
    if (exitAction !== 'idle') return
    setExitAction('saving')
    if (capturing) window.engine.stop()
    if (await saveDraftBeforeExit(true)) {
  ...
  const discardEmptyDraft = async (): Promise<void> => {
    if (exitAction !== 'idle') return
    setExitAction('discarding')
  ...
  useEffect(() => {
    if (!exitDecisionOpen) return
    const onKey = (event: KeyboardEvent): void => {
      if (event.key === 'Escape' && exitAction === 'idle') setExitDecisionOpen(false)
    }
    document.addEventListener('keydown', onKey)
    return () => document.removeEventListener('keydown', onKey)
  }, [exitDecisionOpen, exitAction])
  ...
  return (
    <div className="editor-page">
      ...
      {exitDecisionOpen && (
        <div className="draft-exit-overlay no-drag" role="presentation">
          <div
            className="draft-exit-dialog"
            role="dialog"
            aria-modal="true"
            aria-labelledby="draft-exit-title"
            aria-describedby="draft-exit-copy"
          >
            <h2 id="draft-exit-title">
              Save this empty {meeting?.kind === 'note' ? 'note' : 'meeting'}?
            </h2>
            <p id="draft-exit-copy">
              There isn&rsquo;t a title, note, or transcript yet. You can keep it, discard it, or
              return to editing.
            </p>
            <div className="draft-exit-actions">
              <button
                type="button"
                className="draft-exit-cancel"
                disabled={exitAction !== 'idle'}
                onClick={() => setExitDecisionOpen(false)}
              >
                Keep editing
              </button>
              <button
                type="button"
                className="draft-exit-discard"
                disabled={exitAction !== 'idle'}
                onClick={() => void discardEmptyDraft()}
              >
                {exitAction === 'discarding' ? 'Discarding…' : 'Discard'}
              </button>
              <button
                type="button"
                className="draft-exit-save"
                disabled={exitAction !== 'idle'}
                onClick={() => void saveEmptyDraft()}
                autoFocus
              >
                {exitAction === 'saving' ? 'Saving…' : 'Save'}
              </button>
            </div>
          </div>
        </div>
