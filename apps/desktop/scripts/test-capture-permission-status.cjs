...
async function main() {
  ...
  try {
    ...
    await send({event:'error',message:'Microphone access is off. Open System Settings → Privacy & Security → Microphone.'})
    await send({event:'exit',code:64,signal:null})
    assert.match(await page.locator('body').innerText(),/Microphone access is off/)
