...
## Denied-permission QA follow-up — September 9

Alec confirmed the installed combined preview's normal startup works well. His denied
system-audio test exposed raw `session failed: internalError(...)` and Apple TCC
terminology. `EngineError` now supplies its authored localized message to persistent
sessions. ScreenCaptureKit user-declined failures are recognized by their documented
domain and code at both content lookup and stream start; unrelated failures retain
their own descriptions. Raw diagnostics remain on stderr. Microphone guidance also
omits development-only terminology.

Apple reference: [SCStreamError.userDeclined](https://developer.apple.com/documentation/screencapturekit/scstreamerror/userdeclined).

Native regression: `swiftc -parse-as-library engine/Sources/engine/EngineError.swift engine/tests/EngineErrorTests.swift -o /tmp/doodle-error-tests && /tmp/doodle-error-tests`.
This covers actionable denial, wrong-domain/same-code and non-permission failures,
and authored error messages without Swift enum wrappers. No real TCC state is changed.

The combined local preview also includes ONY-271 / PR #154. Its generation owner needs
to observe capture `ready` before arming automatic notes on finalization. That integration
fix is maintained in the combined preview, with regression coverage for failed startup,
failed Resume, recovery and genuine finalization failures. Carry it into PR #154 or the
combined source before release; ONY-275's independent base does not contain automatic
notes. User QA on the replacement denied case remains required.
